"""Load normalized CNSF data into DuckDB (local) or Databricks (production)."""
...
def load_duckdb(df: pd.DataFrame, db_path: Path, *, mode: str = "replace") -> int:
    ...
    Args:
        df:      Normalized DataFrame from transform.transform().
        db_path: Path to the .duckdb file (created if it doesn't exist).
        mode:    "replace" (default) drops and recreates; "append" inserts rows.
